...
# ロールバック設定
# false : デプロイ失敗時にロールバックする（通常運用の基本）
#         → 更新時は直前の安定した状態へ戻る
#         → 新規作成時は作成途中のリソースを取り消す
# true  : 自動ロールバックを無効にする（デバッグ用途）
#         → 失敗した状態を意図的に残して原因調査したい場合に一時利用
...
>
> - `false` → デプロイ失敗時にロールバック**する**（通常運用の基本。dev / prd ともにこちらを推奨）
> - `true` → 自動ロールバックを**無効にする**（失敗状態を残して調査したい場合などに一時利用）
>
> ロールバックの挙動は、初回スタック作成（CREATE）と既存スタックの更新（UPDATE）で異なります。
>
> | シナリオ | `false`（ロールバックする） | `true`（ロールバックしない） |
> | --- | --- | --- |
> | 初回 CREATE 失敗 | `ROLLBACK_COMPLETE` になり得る | `CREATE_FAILED` で状態を保持 |
> | 既存 Stack の UPDATE 失敗 | `UPDATE_ROLLBACK_COMPLETE` へ戻る | `UPDATE_FAILED` で調査・復旧 |
> | 通常運用 | 基本はこちら | 必要時のみ |
>
> `ROLLBACK_COMPLETE`（初回 CREATE 失敗）と `UPDATE_ROLLBACK_COMPLETE`（UPDATE 失敗後の復旧）は別物です。
> `UPDATE_ROLLBACK_COMPLETE` は前回の安定状態へ戻るだけなので、そのまま原因修正 → 再デプロイできます。
...
| 設定 | 動作 | 用途 |
| --- | --- | --- |
| `false` | デプロイ失敗時にロールバックする | **通常運用の基本**（dev / prd ともに推奨） |
| `true` | 自動ロールバックを無効にする | 失敗状態を意図的に残して原因調査したい場合（デバッグ用途・一時利用） |
...
### エラー1: ROLLBACK_COMPLETE - 初回スタック作成失敗後の状態で止まっている
...
**初回スタック作成（CREATE）途中**でエラーが発生し、CloudFormationによるロールバックが完了した状態です。

`ROLLBACK_COMPLETE` 状態のスタックは更新できないため、一度削除してから再作成します。

> **`ROLLBACK_COMPLETE` と `UPDATE_ROLLBACK_COMPLETE` の違い**
>
> - `ROLLBACK_COMPLETE`：初回 CREATE 失敗 → **削除して再作成が必要**
> - `UPDATE_ROLLBACK_COMPLETE`：既存 Stack の UPDATE 失敗 → 前回の安定状態へ戻るだけ。削除不要で、原因修正後に再デプロイできる
>
> CI/CD で継続運用中のスタックが更新に失敗した場合は `UPDATE_ROLLBACK_COMPLETE` になるため、スタックの削除は不要です。

**対処法**（`ROLLBACK_COMPLETE` の場合）：
...
> **補足**
>
> `disable_rollback = false` はデプロイ失敗時にロールバックする通常運用の設定です。
>
> 初回 CREATE 失敗の調査など、失敗状態を意図的に残したい場合のみ `disable_rollback = true` の一時利用を検討します。
